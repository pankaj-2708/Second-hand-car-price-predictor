...
def fill_mileage(no_of_iter):
    ...
    while(i<no_of_iter+1):
        df.loc[df['Mileage in kmpl or km/kg'].isnull(),['Mileage in kmpl or km/kg']]=df[df['Mileage in kmpl or km/kg'].isnull()].apply(handleNull_mileage,axis=1)['Mileage in kmpl or km/kg']
        i+=1
...
def clean_data(output_path,min_rto_count,min_model_count):
    ...
    df.loc[df['Power in bhp'].isnull(),'Power in bhp']=df[df['Power in bhp'].isnull()].apply(handleNull3,axis=1)
